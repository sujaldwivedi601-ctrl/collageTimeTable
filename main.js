        ...
        semestersToProcess.forEach(className => {
            for (let d = 0; d < 6; d++) {
                ...
                while (cell <= 6) {
                    ...
                    // Single unified pool: any subject with hours left whose teacher is free
                    // RIGHT NOW is a valid candidate — a lone 1-hour block is always allowed.
                    let candidates = workingSubjects[className].filter(s => {
                    ...
                    // Preference order (not exclusion):
                    // 1) late subjects get priority in their allowed window
                    // 2) subjects that CAN safely take a 2-hour block right now get priority
                    //    over ones that can't — but ones that can't are still eligible
                    // 3) more hours remaining first
                    candidates.sort((a, b) => {
                        let aIsLate = isLateSubject(a.name) ? 1 : 0;
                        let bIsLate = isLateSubject(b.name) ? 1 : 0;
                        if (canScheduleLate(d, cell) && aIsLate !== bIsLate) {
                            return bIsLate - aIsLate;
                        }

                        const nextCell = cell + 1;
                        const canPair = (s) =>
                            s.practicalLeft >= 2 &&
                            nextCell <= 6 && nextCell !== 4 &&
                            !currentTeacherOccupancy[`${d}-${nextCell}-${s.teacher}`];

                        let aSafe2 = canPair(a) ? 1 : 0;
                        let bSafe2 = canPair(b) ? 1 : 0;
                        if (aSafe2 !== bSafe2) return bSafe2 - aSafe2;
